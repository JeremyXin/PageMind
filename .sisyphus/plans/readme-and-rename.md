# Plan: Update README for GitHub Upload

## Goal
将 README.md 更新为完整的 PageMind 项目说明文档，然后 git commit & push 到 GitHub。

## Constraints
- README.md 写英文（国际化，适合 GitHub 展示）
- 不修改任何其他代码文件

## TODOs

- [ ] T1: 写入完整 README.md 内容（见下方内容规格）
- [ ] T2: 更新 wxt.config.ts 中的 manifest name 从 "Webpage Summary" 改为 "PageMind"
- [ ] T3: git add + commit（message: `docs: update README for PageMind GitHub launch`）

## README 内容规格（完整内容，T1 直接写入）
...
- [ ] F1: README.md 内容完整，包含所有要求章节
- [ ] F2: wxt.config.ts 中 manifest name 已更新
- [ ] F3: git commit 成功
